  ...
  return (
    <div className="w-full overflow-auto rounded-xl flex flex-col justify-start items-center gap-y-2">
      {tabSections.map((section) => {
        ...
        return (
          <Disclosure
            key={`${section.label}-${isSectionOpen}`}
            as="div"
            className="w-full"
                        ...
                        <div
                          ...
                          className={`tab my-1 cursor-pointer ${
                            selectedTab === keyObj.key
                              ? "tab-selected bg-primary-300"
                              : "tab-hover"
                          }`}
                          onClick={() =>
                            handleChildClick(keyObj.key, section.label)
                          }
                        >
                          <div className="flex items-center">
                            <HomeIcon
                              className={`ml-1 w-5 h-5 text-current ${selectedTab === keyObj.key ? "text-secondary-100" : "text-gray-700"}`}
                            />
