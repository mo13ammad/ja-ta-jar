...
const EditHouseContent = ({
  ...
  const handlePreviousClick = async () => {
    setIsSaving(true); // Start saving
    console.log(
      "Previous button clicked. Starting validation and submission...",
    );
    try {
      if (childRef.current?.validateAndSubmit) {
        const success = await childRef.current.validateAndSubmit();
  ...
  const infoContent = {
    address:
      "اطلاعات مربوط به آدرس اقامتگاه خود را وارد کنید. آدرس اقامتگاه: استان و شهر محل اقامت را وارد کنید. روستا/محله: در صورتی که اقامتگاه در روستا یا محله خاصی است، نام آن را ذکر کنید. اقامتگاه در طبقه: شماره طبقه‌ای که اقامتگاه در آن قرار دارد را وارد کنید. شماره پلاک: شماره پلاک اقامتگاه را درج کنید. کد پستی: کد پستی دقیق اقامتگاه را وارد کنید تا مهمانان بتوانند به‌راحتی محل آن را پیدا کنند.",
    location:
      "بعد از انتخاب استان و نزدیکترین شهر ، محل اقامتگاه خود را در نقشه مشخص نمایید ",
    generalInfo:
      "نوع اجاره و قیمت‌گذاری را انتخاب کنید. نوع اجاره: ۱. اقامتگاه کامل، یعنی کل اقامتگاه یک‌جا اجاره داده شود مانند ویلا و سوئیت. ۲. اتاق، یعنی بخشی یا اتاقی از اقامتگاه اجاره داده شود مثل اتاقی از هتل، اقامتگاه بوم‌گردی، یا مهمان‌خانه. قیمت‌گذاری: ۱. هر شب، یعنی قیمت برای کل اقامتگاه به‌ازای هر شب تعیین شود. ۲. نفر شب، یعنی مبلغ اقامت برای هر نفر در هر شب دریافت شود، مانند برخی اقامتگاه‌های بوم‌گردی و کلبه‌ها.",
    environmentInfo:
      "اطلاعات مربوط به محیط اطراف اقامتگاه خود را تکمیل کنید. بافت محیط: نوع بافت محیطی را انتخاب کنید، مثلاً جنگلی، ساحلی، شهری، و غیره. منظره اقامتگاه: منظره‌ای که اقامتگاه رو به آن دارد را انتخاب کنید، مانند جنگل، کوهستان، دریا، و غیره. توضیحات منظره اقامتگاه: در صورت نیاز، توضیحات بیشتری در مورد منظره اقامتگاه بنویسید. همسایگی: مشخص کنید که همسایگان اقامتگاه از چه نوع مکان‌هایی هستند، مثلاً روستایی یا شهری. مسیر دسترسی: نوع مسیر دسترسی به اقامتگاه، مثلاً آسفالت یا خاکی را مشخص کنید. توضیحات شیوه دسترسی: در صورت نیاز، توضیحاتی درباره نحوه دسترسی به اقامتگاه اضافه کنید.",
    mainFacilities:
    ...
    rooms:
      "میزبان عزیز توجه داشته باشید چند خوابه بودن و فضای خواب اقامتگاه شما به تعداد اضافه شدن اتاق خواب و تخت‌ها در این صفحه بستگی دارد. در صورت انتخاب نوع اجاره بر اساس اتاق در مرحله قبل، اتاق‌های اضافه شده در این بخش مبنای اجاره و قیمت‌گذاری خواهند بود.",
    sanitaries:
