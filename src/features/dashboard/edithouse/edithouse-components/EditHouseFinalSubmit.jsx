...
const EditHouseFinalSubmit = ({ houseId, refetchHouseData }) => {
  ...
  return (
    <div className="flex flex-col justify-end p-4">
      ...
      {/* Confirmation Toggle */}
      <div className="flex items-center shadow-centered px-4  rounded-2xl justify-between mb-4">
        <ToggleSwitch
        ...
        <button
          ...
          className="text-primary-600 underline text-sm px-4 py-3"
        >
          اطلاعات بیشتر
        </button>
      </div>
      ...
      <button
        className={`btn w-44  shadow-centered bg-primary-600 text-white mt-4 ${!confirmationChecked? 'opacity-50' : ''}`}
  ...
  <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-30 p-4">
    <Dialog.Panel className="w-full max-w-md bg-white rounded-2xl p-6">
      <Dialog.Title className="text-lg font-medium">تعهدات میزبان</Dialog.Title>
      <div className="mt-4 overflow-auto max-h-[40vh] scrollbar-thin scrollbar-thumb-primary-500 scrollbar-track-gray-200 p-2">
