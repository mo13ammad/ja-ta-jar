...
const Profile = ({ user, onUpdateUser }) => {
  ...
  return (
    <div className="w-full rounded-xl p-5">
      <div className="border-b pb-3 mb-5 text-xl font-semibold">
        اطلاعات شخصی
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 gap-y-6">
        <UserInfo
          label="نام و نام خانوادگی :"
          value={user?.name || "اطلاعاتی وارد نشده"}
        />
        <UserInfo label="ایمیل :" value={user?.email || "اطلاعاتی وارد نشده"} />
        <UserInfo
          label="شماره تلفن همراه :"
          value={toPersianNumber(user?.phone) || "اطلاعاتی وارد نشده"}
        />
        <UserInfo
          label="کدملی :"
          value={toPersianNumber(user?.national_code) || "اطلاعاتی وارد نشده"}
        />
        <UserInfo label="شهر :" value={cityName} />
        <UserInfo
          label="شماره تلفن همراه دوم :"
          value={toPersianNumber(user?.second_phone) || "اطلاعاتی وارد نشده"}
        />
        <UserInfo label="تاریخ تولد :" value={birthDate} />
        <UserInfo label="جنسیت :" value={sexLabel} />
        <UserInfo
          label="درباره شما :"
          value={user?.bio || "اطلاعاتی وارد نشده"}
        />
      </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <Dialog.Panel className="max-w-lg space-y-4 border bg-white p-6 rounded-3xl w-full">
            <Dialog.Title className="font-bold text-xl">
              آیا از تبدیل شدن به میزبان مطمئن هستید؟
            </Dialog.Title>
            <p className="mt-4">
              در صورت تایید، اطلاعات شما به عنوان میزبان در سیستم ثبت خواهد شد.
            </p>
